...
from aic.elmo_test.elmo_net import LanguageModel
from aic.elmo_test.data import Vocabulary, UnicodeCharsVocabulary, InvalidNumberOfCharacters
...
def train(options, data, n_gpus, tf_save_dir, tf_log_dir,
    ...
    with tf.Session(graph=graph,config=tf.ConfigProto(
        ...
        for batch_no, batch in enumerate(data_gen, start=1):
            ...
            ## each gpu train part of training data
            ## in this step, we assign each model part of the batch
            for k in range(n_gpus):
                model = models[k]
                start = k * batch_size
                end = (k + 1) * batch_size
