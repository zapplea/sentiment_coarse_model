...
class AttributeFunction:
    ...
    def attribute_mat(self, reg, graph):
        ...
        """
        A_mat = tf.get_variable(name='A_mat',initial_value=self.initializer(shape=(self.nn_config['attributes_num'],
        ...
        o_mat = tf.get_variable(name='o_mat',initial_value=self.initializer(shape=(1,
                                                                   self.nn_config['attribute_mat_size'],
                                                                   self.nn_config['attribute_dim']),
                                                            dtype='float32'))
...
class SentimentFunction:
    ...
    def coarse_score(self,fine_score, reg, graph):
        ...
        # shape = (batch size, max review length, coarse attr num*3 + 3)
        coarse_score = tf.reshape(coarse_score,shape=(-1,self.nn_config['max_review_len'],self.nn_config['coarse_attributes_num']*3+3))
        # sahpe = (batch size, coarse attr num*3 + 3)
        coarse_score = tf.reduce_sum(coarse_score,axis=1)
        return tf.reshape(coarse_score,shape=(-1,self.nn_config['coarse_attributes_num']+1,3))
    ...
    def prediction(self,name, score, Y_att, graph):
        ...
        Y_att = tf.tile(tf.expand_dims(Y_att,axis=2),multiples=[1,1,3])
