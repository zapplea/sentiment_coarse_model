...
class SentimentFunction:
    ...
    def sentiment_attention(self, H, W, mask, graph):
        ...
        temp = tf.multiply(mask, tf.exp(tf.clip_by_value(tf.tensordot(H, W, axes=[[-1], [-1]]),
                                                         clip_value_min=tf.constant(-self.nn_config['clip_value']),
                                                         clip_value_max=tf.constant(self.nn_config['clip_value']))))
