    ...
    def __train__(self, dic, graph, gpu_num):
        ...
        for i in range(self.train_config['epoch']):
            ...
            for attr_labels_data, senti_labels_data, sentences_data in dataset:
                ...
                # print(result.shape)
                # print('mask:')
                # result = sess.run(tf.get_collection('mask')[0], feed_dict=feed_dict)
                # print(result.shape)
                # print('score:')
                # result = sess.run(tf.get_collection('score')[0], feed_dict=feed_dict)
                # print(result.shape)
                # print('var_W:')
                # result = sess.run(tf.get_collection('var_W')[0], feed_dict=feed_dict)
                # print(result.shape)
                # print('var_fine_score:')
                # result = sess.run(tf.get_collection('var_fine_score')[0], feed_dict=feed_dict)
                # print(result.shape)
                # print('fine_score:')
