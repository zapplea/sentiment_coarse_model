...
class Classifier:
    ...
    def fine_classifier(self,initializer_A,initializer_O):
        graph = tf.Graph()
        with graph.as_default():
            ...
            if not self.nn_config['is_mat']:
                A, o = self.trans.attribute_vec(initializer_A=initializer_A,initializer_O=initializer_O,graph=graph)
                A = A - o
            else:
                A, o = self.trans.attribute_mat(initializer_A=initializer_A,initializer_O=initializer_O,graph=graph)
                # A.shape = (batch size, words num, attributes number, attribute dim)
                A_lstm = self.af.words_attribute_mat2vec(H, A, graph)
                o_lstm = self.af.words_nonattribute_mat2vec(H, o, graph)
                A_lstm = A_lstm - o_lstm
