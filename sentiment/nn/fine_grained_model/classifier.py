...
class AttributeFunction:
    ...
    def attribute_vec(self,graph):
        ...
        o=tf.get_variable(name='other_vec',initializer=tf.random_uniform(shape=(self.nn_config['word_dim'],),
                                                                         dtype='float32'))
    ...
    def attribute_mat(self,graph):
        ...
        A_mat=[]
        for i in range(self.nn_config['attributes_num']):
            att_mat=tf.get_variable(name='att_mat'+str(i),initializer=tf.random_uniform(shape=(self.nn_config['attribute_mat_size'],
                                                                                               self.nn_config['word_dim']),dtype='float32'))
            A_mat.append(att_mat)
        graph.add_to_collection('A_mat',A_mat)
        o_mat=tf.get_variable(name='other_vec',initializer=tf.random_uniform(shape=(self.nn_config['attribute_mat_size'],
    ...
    def score(self,A_mat,o_mat,x,graph):
        ...
        if not self.nn_config['is_mat']:
            ...
            for i in range(self.nn_config['words_num']):
                ...
                o= o_mat[i]
                A = tf.subtract(A,o)
                temp.append(tf.reshape(tf.matmul(tf.expand_dims(word_embed,axis=0),A,transpose_b=True),shape=(self.nn_config['attributes_num'],)))
    ...
    def loss(self,score,pred,atr_label,graph):
        ...
        condition = tf.equal(pred,atr_label)
        max_wrscore = tf.reduce_max(tf.where(condition,tf.ones_like(atr_label,dtype='float32')*tf.constant(-np.inf,dtype='float32'),score))

        theta=tf.constant(self.nn_config['attribute_loss_theta'],dtype='float32')
        loss=tf.add(tf.subtract(theta,score),max_wrscore)
        # mask loss so that the false label will not be updated.
        masked_loss = tf.multiply(atr_label,loss)
        graph.add_to_collection('atr_loss',masked_loss)
        return masked_loss
...
class SentiFunction:
    ...
    def sentiment_matrix(self,graph):
        W = tf.get_variable(name='senti_mat',initializer= tf.random_uniform(shape=(self.nn_config['normal_senti_prototype_num']*3+self.nn_config['attribute_senti_prototype_num']*self.nn_config['attributes_num'],
    ...
    def sentiment_attention(self,h,W,m,graph):
        ...
        den = tf.tile(den,(1,self.nn_config['normal_senti_prototype_num']*3+self.nn_config['attribute_senti_prototype_num']*self.nn_config['attributes_num']))
    ...
    def relative_pos_matrix(self,graph):
        V = tf.get_variable(name='relative_pos',initializer=tf.random_uniform(shape=(self.nn_config['rps_num'],self.nn_config['rp_dim']),
                                                                              dtype='float32'))
    ...
    def score(self,item1,item2,graph):
        ...
        item2 = tf.reshape(tf.tile(tf.expand_dims(item2,axis=1),[1,3,1]),shape=(3*self.nn_config['attributes_num'],self.nn_config['words_num']))
    ...
    def max_f_senti_score(self,senti_label,score,graph):
        ...
        max_score = tf.reduce_max(tf.where(condition, tf.ones_like(senti_label,dtype='float32')*tf.constant(-np.inf,dtype='float32'),score),axis=1)
        graph.add_to_collection('max_f_senti_score',max_score)
        return max_score
    ...
    def loss(self,senti_label,score,atr_label,graph):
        ...
        :param graph: 
        :return: loss for a sentence for all true attributes and mask all false attributes. shape=(number of attributes,) 
        """
        score = tf.reshape(score,shape=(self.nn_config['attributes_num'],3))
        # item1.shape=(number of attributes,)
        item1 = tf.reduce_sum(tf.multiply(senti_label,score),axis=1)
        #
        item2 = self.max_f_senti_score(senti_label,score,graph)
        loss = tf.add(tf.subtract(self.nn_config['sentiment_loss_theta'],item1),item2)
        masked_loss = tf.multiply(atr_label,loss)
...
class Classifier:
    ...
    def sentences_input(self,graph):
        X=tf.placeholder(shape=(self.nn_config['batch_size'],self.nn_config['sentence_words_num'],self.nn_config['word_dim']),
                         dtype='float32')
    ...
    def sentiment_labels_input(self,graph):
        ...
        """
        y_senti=tf.placeholder(shape=(self.nn_config['batch_size'],self.nn_config['attributes_num'],3))
    ...
    def senti_extors_input(self,graph):
        ...
        shape of the matrix: [batch_size, attribute numbers, sentiment epression dim(sentiment dim)]
        :param graph: 
        :return: 
        """
    ...
    def sentence_lstm(self,x,graph):
        ...
        :return: 
        """
        weight = tf.get_variable(name='sentence_lstm_w',initializer=tf.random_uniform(shape=(self.nn_config['word_dim'],
                                                                                             self.nn_config['lstm_cell_size']),
                                                                                      dtype='float32'))
        bias = tf.get_variable(name='sentence_lstm_b', initializer=tf.zeros(shape=(self.nn_config['lstm_cell_size']),dtype='float32'))

        xt = tf.add(tf.expand_dims(tf.matmul(x,weight),axis=0),bias)
        cell = tf.nn.rnn_cell.BasicLSTMCell(self.nn_config['lstm_cell_size'])
        init_state = cell.zero_state(batch_size=1,dtype='float32')
        # outputs.shape = (max time, batch size, cell size)
        outputs, _ = tf.nn.dynamic_rnn(cell, inputs=xt,initial_state=init_state)
        # outputs.shape = (batch size, max time, cell size)
    ...
    def optimizer(self,joint_loss,graph):
        opt=tf.train.AdamOptimizer(self.nn_config['lr']).minimize(tf.reduce_mean(joint_loss))
        ...
        for i in range(self.nn_config['batch_size']):
            with graph.as_default():
                ...
                with tf.variable_scope('sentence_lstm',reuse=reuse):
                    # h.shape = (max time, 1(=batch size),cell size)
                    h = self.sentence_lstm(x,graph=graph)
                    h = tf.reshape(h,shape=(self.nn_config['wrods_num'],self.nn_config['lstm_cell_size']))
                atr_label = y_att[i]
            ...
            # sentiment function
            with graph.as_default():
                # extract sentiment expression corresponding to sentiment and attribute from W
                # for all attributes
            ...
            # Joint Loss
            with graph.as_default():
                joint_loss.append(tf.reduce_mean(tf.add(senti_loss,atr_loss)))

        with graph.as_default():
            opt=self.optimizer(joint_loss=joint_loss)
