...
class AttributeFunction:
    ...
    def loss(self, score, pred, atr_label, graph):
        ...
        loss = tf.add(tf.subtract(theta, score), max_fscore)
...
class SentiFunction:
    ...
    #     loss = tf.add(tf.subtract(theta, item1), item2)
    #     masked_loss = tf.multiply(atr_label, loss)
...
class Classifier:
    ...
    def reviews_input(self, graph):
        ...
        """
        X = tf.placeholder(
            shape=(self.nn_config['review_batch_size'],self.nn_config['sentences_num'], self.nn_config['sentence_words_num']),
            dtype='float32')
        graph.add_to_collection('X', X)
        return X

    def review_weight_mask_input(self,graph):
        """
        This function input a mask when calculate p(x|a,y). It eliminate influence of padding sentences. This mask is calculated outside the model.
        for padding sentence, it is zeros at that position.
        :param graph: 
        :return: 
        """
        mask = tf.placeholder(shape=(self.nn_config['review_batch_size'],self.nn_config['sentences_num']),dtype='float32')
        graph.add_to_collection('review_weight_mask',mask)
        return mask
    ...
    def sentence_lstm(self, X, graph):
        ...
        # outputs.shape = (sentences num, max_time, cell size)
    ...
    # attribute score function related
    def attribute_labels_input(self, graph):
        y_att = tf.placeholder(shape=(self.nn_config['review_batch_size'], self.nn_config['attributes_num']), dtype='float32')
        graph.add_to_collection('y_att', y_att)
        return y_att

    # sentiment score function related
    def sentiment_labels_input(self, graph):
        """
        :param graph: 
        :return: shape=[batch_size, number of attributes, 3], thus ys=[...,sentence[...,attj_senti[0,1,0],...],...]
        """
        y_senti = tf.placeholder(shape=(self.nn_config['review_batch_size'], self.nn_config['attributes_num'], 3),dtype='float32')
    ...
    def senti_extors_input(self, graph):
        ...
        extors = tf.placeholder(shape=(3 * self.nn_config['attributes_num'],
                                       self.nn_config['normal_senti_prototype_num'] * 3 + self.nn_config[
                                           'attribute_senti_prototype_num'] * self.nn_config['attributes_num'],
                                       self.nn_config['sentiment_dim']), dtype='float32')
    ...
    def classifier(self):
        ...
        with graph.as_default():
            # shape = (review batch size, sentence numbers, words number)
            R = self.reviews_input(graph=graph)
            # shape = (review batch size, sentence numbers, word dim)
            R = self.lookup_table(R, graph)
            # input aspect labels for each sentence in a review
            # shape = (review batch size, attributes num). one hot. For each review there might be more than one attribute label.
            y_att = self.attribute_labels_input(graph=graph)
            # shape = (review batch size, attributes num, 3). one hot
            ...
            #review weight mask.shape = (review batch size, sentences number)
            rw_M = self.review_weight_mask_input(graph)
            # ???one problem to calculate sr is that we need to use hl to calculate its aspect.
            joint_losses = []
        # extract sentence from each review batch
        for k in range(self.nn_config['review_batch_size']):
            with graph.as_default():
                X = R[k]
                # lstm
                with tf.variable_scope('sentence_lstm'):
                    ...
                    Hl = tf.reshape(tf.transpose(H,[1,0,2])[-1],shape=(self.nn_config['sentences_num'],
                                                                       self.nn_config['lstm_cell_size']))
                    # calculate weight of each sentences for aspect in review
                    # need to consider that there are multiple aspect in a review, so calculate review for all aspects and eliminate the false aspect's loss
                atr_label = y_att[k]
                senti_label = y_senti[k]
                # calculate s(x; a, y)
                rw_mask = rw_M[k]
                # the first item to calculate is convert A_mat to A_vec for each
