...
class MultifilterTrain:
    ...
    def train(self,classifier):
        ...
        with graph.as_default():
            ...
            score_lstm = graph.get_collection('score_lstm')[0]
            keep_prob_f = graph.get_collection('keep_prob_f')[0]
            keep_prob_lstm = graph.get_collection('keep_prob_lstm')[0]
            # attribute function
            init = tf.global_variables_initializer()

            max_train_macro_f1 = 0
            max_train_micro_f1 = 0
            max_test_macro_f1 = 0
            max_test_micro_f1 = 0
            ...
            with tf.Session(graph=graph, config=config) as sess:
                ...
                for i in range(self.nn_config['epoch']):
                    ...
                    for j in range(batch_num):
                        ...
                        _, train_loss, TP_data, FP_data, FN_data, pred_data, score_data, score_pre_data, score_lstm_data \
                            = sess.run(
                            [train_step, loss, TP, FP, FN, pred, score, score_pre, score_lstm],
                            feed_dict={X: sentences, Y_att: Y_att_data, keep_prob_f: self.nn_config['keep_prob_f'], keep_prob_lstm: self.nn_config['keep_prob_lstm']})
                    ...
                    #     if max_train_macro_f1 < np.mean(_f1_score):
                    #         max_train_macro_f1 = np.mean(_f1_score)
                    #     print('max_train_macro_f1:',max_train_macro_f1)
                    ...
                    #
                    #     if max_train_micro_f1 < _f1_score:
                    #         max_train_micro_f1 = _f1_score
                    #     print('max_train_micro_f1:', max_train_micro_f1)
                    #     # # np.random.seed(1)
                    ...
                    if i % 1 == 0 and i != 0:
                        ...
                        for i in range(valid_size // batch_size):
                            ...
                            for n in range(self.nn_config['batch_size']):
                                ...
                                score_vec.append(score_data[n])
                                score_pre_vec.append(score_pre_data[n])
                        print('Test loss:%.10f' % np.mean(loss_vec))
                        ...
                        if max_test_macro_f1 < np.mean(_f1_score):
                            max_test_macro_f1 = np.mean(_f1_score)
                        print('max_test_macro_f1:',max_test_macro_f1)
                        _precision = self.mt.precision(TP_vec, FP_vec, 'micro')
                        _recall = self.mt.recall(TP_vec, FN_vec, 'micro')
                        _f1_score = self.mt.f1_score(_precision, _recall, 'micro')
                        print('Micro F1 score:', _f1_score, ' Micro precision:', np.mean(_precision), ' Micro recall:', np.mean(_recall))

                        if max_test_micro_f1 < _f1_score:
                            max_test_micro_f1 = _f1_score
                        print('max_test_micro_f1:', max_test_micro_f1)
                        # np.random.seed(1)
                        # check_num = 1
