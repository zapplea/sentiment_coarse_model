...
class SentiTrain:
    ...
    #
    #         senti_score = graph.get_collection('senti_score')[0]
    ...
    #         print(self.dg.aspect_dic,self.dg.senti_dic)
    ...
    #                     _ , loss_data ,senti_score_data, TP_data, FP_data, FN_data= sess.run([train_step,loss,senti_score,TP , FP , FN], feed_dict={X: sentences, Y_att: Y_att_data, Y_senti: Y_senti_data,keep_prob_lstm: self.nn_config['keep_prob_lstm']})
    #
    #                     print(i,j,loss_data)
    #                     print(TP_data,FP_data,FN_data)
    #                     # print(check_data.shape,check_data)
    #                 if i % 5000 == 0 and i != 0:
    #                     valid_size = self.dg.val_data_size
    #                     p = 0
    #                     l = 0
    #                     count = 0
    ...
    #                         count += 1
    #                         p += sess.run(loss, feed_dict={X: sentences,
    #                                                        Y_att: Y_att_data,
    #                                                        Y_senti: Y_senti_data,
    #                                                        keep_prob_lstm: 1.0})
    #                         l += sess.run(loss, feed_dict={X: sentences,
    #                                                        Y_att: Y_att_data,
    #                                                        Y_senti: Y_senti_data,
    #                                                        keep_prob_lstm: 1.0})
    #                     p = p / count
    ...
    def train(self,classifier):
        ...
        with graph.as_default():
            ...
            FP = graph.get_collection('FP')[0]
            keep_prob_lstm = graph.get_collection('keep_prob_lstm')[0]
            word_score = graph.get_collection('word_score')[0]
        ...
        with graph.device('/gpu:1'):
            ...
            with tf.Session(graph=graph, config=config) as sess:
                ...
                for i in range(self.nn_config['epoch']):
                    ...
                    for j in range(batch_num):
                        sentences, Y_att_data ,Y_senti_data= self.dg.data_generator(j,'train')
                        _, train_loss, TP_data, FP_data, FN_data, pred_data, score_data ,check_data,word_score_data\
                            = sess.run(
                            [train_step, loss, TP, FP, FN, pred, score ,check,word_score],
