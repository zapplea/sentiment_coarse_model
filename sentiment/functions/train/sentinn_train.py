...
class SentiTrain:
        ...
        with graph.device('/gpu:1'):
            ...
            with tf.Session(graph=graph, config=tf.ConfigProto(allow_soft_placement=True)) as sess:
                ...
                for i in range(self.nn_config['epoch']):
                    sentences, Y_att_data, Y_senti_data = self.dg.data_generator(i,'train')
                    _ , loss_data = sess.run([train_step,loss], feed_dict={X: sentences, Y_att: Y_att_data, Y_senti: Y_senti_data,keep_prob_lstm: self.nn_config['keep_prob_lstm']})
    ...
    #                     sentences, Y_att_data ,Y_senti_data = self.dg.train_data_generator(j)
    #                     _, train_loss, TP_data, FP_data, FN_data, pred_data, score_data \
    #                         = sess.run(
    #                         [train_step, loss, TP, FP, FN, pred, score],
    ...
    #
    #                 if i % 1 == 0 and i != 0:
    #                     sentences, Y_att_data , Y_senti_data = self.dg.test_data_generator()
