...
class RelScore:
    ...
    def relevance_prob_senti(self, senti_score, Y_senti, graph):
        ...
        senti_score = tf.reshape(senti_score, shape=(
        -1, self.nn_config['max_review_length'], self.nn_config['attributes_num'] + 1, 3))
    ...
    def softmax_loss(self, labels, logits, senti_rel_prob, aspect_prob, graph):
        ...
        loss = tf.reduce_mean(tf.add(
            tf.reduce_sum(tf.multiply(aspect_prob, tf.multiply(senti_rel_prob,
