...
class AttributeFunctionTest(unittest.TestCase):
    ...
    def test_loss(self):
        ...
        # for i in range(self.nn_config['batch_size']):
        #     item1[i][0] = 1
        # nonatr_mask = tf.Session().run(tf.where(condition, tf.constant(item1, dtype='float32'), Y_att))
        #
        # test_max_fscore = np.ones_like(result,dtype='float32')
        # for i in range(0,int(math.ceil(self.nn_config['batch_size']/3))):
        #     test_max_fscore[i] = int(math.ceil(self.nn_config['attributes_num']/3))-1
        #
        # for i in range(int(math.ceil(self.nn_config['batch_size']/3)),int(math.ceil(self.nn_config['batch_size']*2/3))):
        #     test_max_fscore[i] = int(math.ceil(self.nn_config['attributes_num']*2/3))-1
        # for i in range(int(math.ceil(self.nn_config['batch_size']*2/3)),self.nn_config['batch_size']):
        #     test_max_fscore[i] = self.nn_config['attributes_num']-1
        #
        # test_data = np.multiply(np.add(np.subtract(self.nn_config['attribute_loss_theta'],np.multiply(Y_att,score)),test_max_fscore),nonatr_mask)
        # self.assertTrue(np.all(test_data == result))
