...
class Classifier:
    ...
    def sentence_weight(self,relevance_score,mask,graph):
        ...
        relevance_weight = tf.divide(exp_score,tf.tile(tf.reduce_sum(exp_score,axis=1,keep_dims=True),multiples=[1,self.nn_config['sentences_num']]))
    ...
    def classifier(self):
        ...
        for k in range(self.nn_config['batch_size']):
            with graph.as_default():
                ...
                sentence_relevance_weight = self.sentence_weight(relevance_score=relevance_score, mask=ispad, graph=graph)
